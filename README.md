...
已实现真正的 Windows 虚拟扩展桌面，通过 USB 在安卓设备显示。Windows 程序的“启动 USB 副屏”会请求管理员授权，创建扩展屏、定位输出并连接手机；使用已安装的测试签名 IDD 驱动，尚非通用驱动安装器。用户已确认集成入口可用。

- **兼容路径**：小米 15S Pro 的 20 秒动态扩展桌面短测正常完成，发送 1183 帧，去掉前五次统计后平均解码 59.93 fps。不能据此宣称长期稳定或物理屏幕级延迟。
- **Android 0.3.1**：美化控制面板，支持隐藏/恢复、折叠详细统计、保持亮屏开关及偏好记忆；隐藏时提示连接状态，支持滚动与宽度适配。已构建安装，完整点击与 Tab S4 布局验收仍待确认。
- **正确性修正**：显式提交 SPS/PPS 初始化、保留 MFT 输入事件额度、按要求对齐输出缓冲。离线检查与独立解码回归通过。
- **启动过载**：四帧队列增加最多 100 ms 的发布等待，不丢参考帧；NVENC 测试画面短测通过，但不保证全链路积压上界。
- **原生 native-mf 路径仍为实验功能**：已接通 IDD GPU 纹理到硬件编码再到 USB，实机仍约 27–36 fps 并积压，尚未解决。不要把它当作已完成的低延迟替代路径。

延迟记录区分发送等待、socket 写入和手机回调确认；回调确认不是屏幕亮起时间，socket 写入不是 USB 送达。尚需长期稳定性、实际亮屏延迟及三星 Tab S4 验收。
