...
通过 **ADB USB** 将 Windows 画面或指定视频显示在 Android 平板上，不使用 Wi-Fi、USB 网络共享或浏览器。当前版本已经具备 Windows 虚拟扩展桌面、桌面复制、指定视频播放、Android 接收端、低延迟 EGL 最新帧渲染和断线后自动等待重连。
...
4. 启动 Windows 程序 `WiredScreen.exe`，选择用途：**扩展桌面**、**复制桌面**、**播放指定视频文件**或**动态测试画面**。
5. 为获得最佳延迟体验，选择 **“低延迟 V2（推荐）”** 和 **`libx264`** 编码器。
6. 根据平板性能选择 1920×1080、1600×900 或 1280×720，以及 60、50 或 30 fps；码率也会实际传给编码器。
7. 先用“动态测试画面”确认手机端解码和呈现统计接近目标帧率。
8. 扩展或复制模式会请求管理员权限并创建虚拟显示器。扩展模式可在 Windows 显示设置中排列新屏幕；复制模式会镜像现有桌面。指定视频文件会循环播放到平板，当前不传输音频。
