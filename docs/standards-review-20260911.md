...
### 2026-09-12 用户报告启动 25 帧即停止

usb-20260912-090424.jsonl 证实本轮为 testsrc2 + h264_nvenc，非原生路径；手机已初始化并返回首批帧确认。四帧队列满即失败，对短暂启动/调度突发过于敏感。

调整为队列满时最多等待 100 ms，让消费者释放空间；容量仍为四帧、不丢参考帧，连续满队列超时仍失败。此等待只限制单次发布阻塞，不保证全链路积压或亮屏延迟上界，持续性能问题仍需处理。

同样 --test --encoder h264_nvenc --seconds 15 短测正常结束，发送 849 帧，无过载；证据 usb-20260912-090611.jsonl。不宣称单轮测试已解决所有启动失败。原 WiredScreen.exe 被用户正在打开的窗口占用，修正版为 native/dist/WiredScreen.QueueFix.exe；默认文件未覆盖。

### 2026-09-12 发送路径延迟排查

合并包头与负载为单次写入的试验（usb-20260912-090948.jsonl）未显示改善，已撤回。该试验文件 WiredScreen.SendTest.exe 不作为推荐版本。

新增 send 记录，统计包读取完毕至发送的等待、socket 写入耗时、包字节数；analyze_latency.py 支持按实际 PC 时钟预热五秒后汇总。注意它不能覆盖编码前和未从管道读取的积压，socket 写完也不等于 USB 已送达。

WiredScreen.Latency.exe 使用恢复后的两次写入方式和 100 ms 队列背压，testsrc2 + NVENC 15 秒实测发送 917 帧正常结束。usb-20260912-091058.jsonl：readyToSend 中位 0.0434 ms / P95 0.2593 ms；write 中位 0.1907 ms / P95 0.5032 ms；packetReadyToAck 中位 19.63 ms / P95 28.05 ms。这些是局部测试画面数据，不是完整扩展桌面或亮屏延迟验收。未发现此轮稳定阶段的电脑发送队列是主要瓶颈；本轮不宣称已降低实际显示延迟。

### 2026-09-12 集成副屏入口

Windows 界面新增“启动 USB 副屏”：非管理员时通过 runas 重新启动同一应用并带 --ui-extend，Windows 授权后自动创建虚拟显示设备、启用扩展桌面、等待 DXGI 输出就绪、启动 USB 会话。取消 UAC 时保留原窗口并提示取消。传输中禁用移除副屏和输入选项，创建时避免关闭窗口与设备初始化并发；停止传输后可移除副屏。

不再把所有创建设备失败都归为驱动未安装。此功能使用已安装驱动，不自动构建或安装驱动。独立程序 native/dist/WiredScreen.Integrated.exe 编译和协议自检通过；本轮尚未完成点击界面的完整 UAC/创建/手机显示验收。
